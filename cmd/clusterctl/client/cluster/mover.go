...
type ObjectMover interface {
	...
	Move(namespace string, toCluster Client, clusterName string, dryRun bool, mutators ...ResourceMutatorFunc) error
	...
	// If `clusterName` is specified (not empty string), only objects belonging to the cluster will be moved.
	ToDirectory(namespace string, directory string, clusterName string) error

	// FromDirectory reads all the Cluster API objects existing in a configured directory to a target management cluster.
	// If `clusterName` is specified (not empty string), only objects belonging to the cluster will be moved.
	FromDirectory(toCluster Client, directory string, clusterName string) error
}
...
func (o *objectMover) Move(namespace string, toCluster Client, clusterName string, dryRun bool, mutators ...ResourceMutatorFunc) error {
	log := logf.Log
...
func (o *objectMover) ToDirectory(namespace string, directory string, clusterName string) error {
	log := logf.Log
